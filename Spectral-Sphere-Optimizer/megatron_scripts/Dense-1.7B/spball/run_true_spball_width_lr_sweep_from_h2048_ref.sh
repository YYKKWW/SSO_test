...
# Single-GPU reproduction wrapper for the true SpBall h2048 reference log.
# It keeps the reference architecture rule:
#   layers=28, seq=4096, head_dim=128, ffn_hidden=3*hidden,
#   and sweeps width/lr under the same Spectral Ball defaults.
#
# Start with DRY_RUN=1 to print the manifest. Use SMOKE=1 for a quick check.

WORKSPACE=${WORKSPACE:-/workspace}
SESSION_NAME=${SESSION_NAME:-true_spball_mup_width_lr_sweep_from_h2048_ref}
REPO_PATH=${REPO_PATH:-${WORKSPACE}/results/optimizer_arena_v2/true_spball_mup_width_lr_sweep_from_h2048_ref}
...
if [ "$SMOKE" = "1" ]; then
    echo "Running a single true-SpBall reference smoke test: width=256, ${TRAIN_ITER:-10} iterations."
    OPTIMIZER=${OPTIMIZER:-spectral_ball_dist} \
    WIDTH=${WIDTH:-256} \
    ...
    JOB_NAME=${JOB_NAME:-true_spball_ref_width256_smoke} \
    ...
    bash "${SCRIPT_DIR}/spball.sh"
    exit 0
fi

echo "Single-GPU true-SpBall width/lr sweep from h2048 reference"
echo "  widths: ${WIDTHS}"
echo "  lrs: ${LRS}"
...

{
    ...
    for width in $WIDTHS; do
        for lr in $LRS; do
            job_lr=${lr//./p}
            job_lr=${job_lr//-/m}
            job_name="mup_spectral_ball_dist_h${width}_lr${job_lr}"
            log_file="${REPO_PATH}/logs/${job_name}.log"
            printf "spectral_ball_dist\t%s\t%s\t%s\t%s\n" "$width" "$lr" "$job_name" "$log_file"
            run_id=$((run_id + 1))
        done
    done
} > "$MANIFEST"
