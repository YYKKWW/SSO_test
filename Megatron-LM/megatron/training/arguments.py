...
def validate_args(args, defaults={}):
    ...
    # SpectralBall / SpEL optimizer checks
    if args.optimizer in ('spectral_ball', 'spectral_ball_dist', 'spel', 'spel_dist'):
        optimizer_name = args.optimizer
        assert not args.use_distributed_optimizer, f"{optimizer_name} optimizer does not support distributed optimizer for now."
        assert not args.use_torch_fsdp2, f"{optimizer_name} optimizer does not support Torch-FSDP2 for now."
        assert not args.use_megatron_fsdp, f"{optimizer_name} optimizer does not support Megatron-FSDP for now."
        assert args.ckpt_format in ["torch", "torch_dist"], f"{optimizer_name} optimizer supports torch and torch_dist checkpoint format."
...
def _add_regularization_args(parser):
    ...
    group.add_argument('--spel-momentum', type=float, default=0.9,
                       help='Momentum coefficient for SpEL optimizer')
    group.add_argument('--spel-use-nesterov', action='store_true', default=True,
                       help='Use Nesterov-style momentum in SpEL')
    group.add_argument('--spel-no-split-qkv', action='store_false', default=True,
                       dest='spel_split_qkv',
                       help='Whether to split QKV parameters for SpEL optimizer')
    group.add_argument('--spel-qkv-split-mode', type=str, default='component',
                       choices=['component', 'group', 'head'],
                       help='QKV split mode for SpEL: component (merge all groups Q/K/V), '
                            'group (per query group), or head (per attention head)')
    group.add_argument('--spel-no-split-fc1', action='store_false', default=True,
                       dest='spel_split_fc1',
                       help='Disable splitting FC1 (gate and up) for gated linear units (SwiGLU) in SpEL.')
    group.add_argument('--spel-no-split-moe-experts', action='store_false', default=True,
                       dest='spel_split_moe_experts',
                       help='Disable splitting MoE experts for SpEL optimizer.')
    group.add_argument('--spel-msign-steps', type=int, default=8,
                       help='Number of Newton-Schulz iteration steps for matrix sign function in SpEL')
    group.add_argument('--spel-radius-mode', type=str, default='spectral_mup',
                       choices=['spectral_mup', 'identity', 'initialize'],
                       help='Mode for computing target radius R in SpEL')
    group.add_argument('--spel-power-iteration-steps', type=int, default=10,
                       help='Number of power iteration steps for computing spectral norm in SpEL')
    group.add_argument('--spel-scale-mode', type=str, default='spectral_mup',
                       choices=['align_adamw_rms', 'spectral_mup', 'shape_scaling'],
                       help='Scale mode for SpEL optimizer (mirrors MuonBall/SpectralBall scale modes)')
    group.add_argument('--spel-retract-mode', type=str, default='hard',
                       choices=['hard', 'dynamic'],
                       help='Retraction mode for SpEL: hard (project to sphere) or dynamic (gradual adjustment)')
    group.add_argument('--spel-retract-alpha', type=float, default=0.05,
                       help='Step size for dynamic retraction mode in SpEL (ignored for hard mode)')
...
def _add_training_args(parser):
    ...
    group.add_argument('--optimizer', type=str, default='adam',
                       choices=['adam', 'sgd', 'muon', 'dist_muon', 'muon_ball', 'muon_ball_dist', 'spel', 'spel_dist', 'spectral_ball', 'spectral_ball_dist'],
