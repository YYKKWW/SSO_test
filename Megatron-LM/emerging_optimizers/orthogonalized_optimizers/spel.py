...
"""SpEL optimizer with spectral tangent-plane projection.
...
class SpEL(OrthogonalizedOptimizer):
    ...
    def __init__(
        ...
        *,
        ...
        # Store SpEL specific parameters
        ...
        # SpEL uses custom orthogonalize() method instead
        def scaled_orthogonalize_fn(grad: torch.Tensor) -> torch.Tensor:
            raise NotImplementedError(
                "SpEL uses custom orthogonalize() method. "
    ...
    def _compute_component_update(
        ...
        """Compute SpEL update for a single Q/K/V component.
    ...
    def orthogonalize(self, p: torch.Tensor, grad: torch.Tensor, **kwargs: Any) -> torch.Tensor:
        """Compute SpEL update direction.

        This method overrides the base class orthogonalize() to implement the SpEL
        ...
        if self.split_fc1 and self.is_fc1_fn is not None and self.is_fc1_fn(p):
            ...
            # Compute SpEL update for each component
        ...
        update, bias, sigma = compute_spel_update(
            W=p.data,
            M=grad,
